...
这里也有一个微妙的方面。我们选择的空闲链表格式（*它的序言块和结尾块总是标记为已分配*）允许我们忽略潜在的麻烦边界情况，也就是，请求块 bp 在堆的起始处或者是在堆的结尾处。
如果没有这些特殊块，代码将混乱得多，更加容易出错，并且更慢，因为我们将不得不在每次释放请求时，都去检査这些并不常见的边界情况。
> Dummy 编程技巧

## 5. 分配块
一个应用通过调用 mm_malloc 函数（见图 9-47）来向内存请求大小为 size 字节的块。
在检査完请求的真假之后，分配器必须调整请求块的大小，从而为头部和脚部留有空间，并满足双字对齐的要求。
第 12 ~ 13 行强制了最小块大小是 16 字节：8 字节用来满足对齐要求，而另外 8 个用来放头部和脚部。
对于超过 8 字节的请求（第 15 行），一般的规则是加上开销字节，然后向上舍入到最接近的 8 的整数倍。

~~~c
// mm.c
// mm_malloc：从空闲链表分配一个块
/**
 * Allocates a block of memory of at least 'size' bytes and returns a pointer to it.
 *
 * @param size The size of the memory block in bytes to allocate.
 * @return A pointer to the allocated memory block or NULL if the allocation fails.
 */
void *mm_malloc(size_t size)
{
    size_t asize;      /* Adjusted block size to meet alignment requirements */
    size_t extendsize; /* Size to extend the heap by if there is no suitable free block */
    char *bp;          /* Pointer to the block of allocated memory */

    /* If the requested size is 0, return NULL as per standard malloc behavior */
    if (size == 0)
        return NULL;

    /**
     * Adjust the block size to include overhead (metadata, etc.) and to meet
     * alignment requirements. The minimum block size is twice the 'DSIZE',
     * which is typically the size of two words (to store metadata like size
     * and whether the block is free or not).
     */
    if (size <= DSIZE)
        asize = 2 * DSIZE;
    else
    ...
    /**
     * Search the free list for a block of memory that fits the adjusted size
     * requirements. If a suitable block is found, allocate ('place') the block,
     * update metadata as necessary, and return a pointer to the allocated block.
     */
    ...
    /**
     * If no suitable block is found in the free list, extend the heap by the
     * greater of the adjusted size and a predefined 'CHUNKSIZE'. This ensures
     * that we either get enough space for the current request or add a
     * substantial amount of memory to the heap to accommodate future requests.
     */
    extendsize = MAX(asize, CHUNKSIZE);
    if ((bp = extend_heap(extendsize / WSIZE)) == NULL)
        return NULL;  // Return NULL if the heap cannot be extended

    /**
     * Once the heap is extended, allocate the requested memory block by updating
     * the metadata of the newly added space and return a pointer to this block.
     */
    place(bp, asize);
    return bp;
}
...
一旦分配器调整了请求的大小，它就会搜索空闲链表，寻找一个合适的空闲块（第 18 行）。
如果有合适的，那么分配器就放置这个请求块，并可选地分割出多余的部分（第 19 行），然后返回新分配块的地址。
...
~~~
这又是一个帮助你熟悉分配器的热身练习。注意对于这个分配器，最小块大小是 16 字节。
如果分割后剩下的块大于或者等于最小块大小，那么我们就分割这个块（第 6 ~ 10 行）。
这里唯一有技巧的部分是要意识到在移动到下一块之前（第 8 行），你必须放置新的已分配块（第 6 行和第 7 行）。
...
====================================================================================================
这句话讨论的是隐式空闲链表在内存分配中的应用，特别是其效率问题。让我们分解这句话以更好地理解它的含义：

1. **隐式空闲链表**：这是一种内存管理技术，用于跟踪计算机内存中的空闲空间。在这种方法中，空闲的内存块被组织成一个链表，每个块包含了指向下一个空闲块的信息。这种方法的一个特点是它不需要额外的数据结构来跟踪空闲块；所有必要的信息都存储在空闲块内部。

2. **基本分配器概念**：隐式空闲链表提供了一种简单的方式来实现内存分配器，它介绍了分配和回收内存的基本概念，如何跟踪空闲内存，以及如何将内存分配给需要的进程。

3. **块分配与堆块的总数呈线性关系**：这意味着隐式空闲链表的效率问题。当你尝试在堆中分配一个块时，你可能需要遍历整个链表来找到足够大的空闲块。随着堆中块的数量增加，这个查找过程的时间也线性增加。这是因为每个新的分配请求可能需要检查更多的块。

4. **不适合通用的分配器**：由于这种线性关系，隐式空闲链表不适合用作通用的内存分配器，特别是在那些可能有大量内存块的系统中。在这些系统中，分配器的效率是一个重要的考量，因为效率低下可能导致性能问题。

5. **适用于堆块数量很小的特殊分配器**：对于那些堆块数量相对较少的场景，隐式空闲链表可能是一个合适的选择。在这些情况下，遍历整个链表找到一个合适的空闲块的开销不会太大。

简而言之，这句话的意思是，虽然隐式空闲链表是一种简单的内存管理技术，适用于介绍基本的内存分配概念，但由于其在处理大量内存块时效率低下，它通常不适用于需要高效内存管理的通用分配器。然而，在那些内存块数量相对较少的特定应用中，它可能是一个可行的选择。
====================================================================================================

一种更好的方法是将空闲块组织为某种形式的显式数据结构。
因为根据定义，程序不需要一个空闲块的主体，所以实现这个数据结构的指针可以存放在这些空闲块的主体里面。
例如，堆可以组织成一个双向空闲链表，在每个空闲块中，都包含一个 `pred`（前驱）和 `succ`（后继）指针，如图 9-48 所示。
...
====================================================================================================
这段话描述的是使用显式数据结构来优化内存分配器的效率，具体是通过实现一个双向空闲链表来替代隐式空闲链表。
以下是这种方法的优势和工作原理的详细解释：

1. **显式数据结构的使用**：在这种方法中，空闲块被组织成一个显式的数据结构，如双向链表。
这意味着每个空闲块都显式地包含了指向其他空闲块的指针，而不是隐式地通过块的大小或其他方式来确定。

2. **存储在空闲块的主体里面**：由于程序不需要使用空闲块的主体（即实际的存储空间），因此这部分空间可以用来存储管理空闲块所需的元数据，如指向前一个（`pred`）和后一个（`succ`）空闲块的指针。这样做既节省了额外的存储空间，又使得空闲块的管理更为高效。

3. **双向空闲链表**：与隐式空闲链表相比，双向链表允许从任一方向遍历空闲块。这种结构使得在查找、分配和回收内存块时更加灵活和高效。

4. **减少分配时间**：在隐式空闲链表中，分配器可能需要遍历整个堆以找到一个合适的空闲块，导致分配时间与堆中块的总数成线性关系。
而在双向空闲链表中，分配器只需遍历空闲块，而非所有块。因此，分配时间从与堆中所有块的总数成线性关系减少到与空闲块数量成线性关系。这在空闲块数量远少于总块数的情况下尤为有效，大大提高了分配器的效率。

5. **首次适配的优化**：在使用双向空闲链表时，首次适配策略（即选择第一个足够大的空闲块进行分配）变得更加高效。
因为分配器不需要检查每个块，而只需检查空闲块，所以找到合适的空闲块所需的时间会大大减少。

综上所述，通过将空闲块组织成双向链表，内存分配器在分配内存时的效率得到显著提升，尤其是在首次适配策略下。
这是因为分配时间变得只与空闲块的数量成线性关系，而非整个堆的总块数。
====================================================================================================

一种方法是用后进先出（LIFO）的顺序维护链表，将新释放的块放置在链表的开始处。
使用 LIFO 的顺序和首次适配的放置策略，分配器会最先检查最近使用过的块。在这种情况下，释放一个块可以在常数时间内完成。
如果使用了边界标记，那么合并也可以在常数时间内完成。

====================================================================================================
这段话描述了在内存分配器中使用后进先出（LIFO）顺序维护空闲块链表的方法，以及如何通过这种方法提高内存分配和释放的效率。以下是关键点的详细解释：

1. **后进先出（LIFO）的顺序**：在这种方法中，当一个内存块被释放时，它被放置在空闲链表的开始处。
这意味着最后被释放的块将是第一个被考虑用于新的分配请求的块。由于新释放的块可能仍然在缓存中，这可能提高了内存访问的效率。

2. **首次适配的放置策略**：这是一个常见的内存分配策略，指的是分配器在分配内存时会选择它遇到的第一个足够大的空闲块。
结合LIFO顺序，这意味着分配器将首先检查最近释放的块，这些块更可能仍然在CPU的缓存中，从而提高访问速度。

3. **释放块的常数时间操作**：在LIFO策略下，释放一个块仅涉及将其放置在空闲链表的头部。
这是一个常数时间操作，因为它不依赖于堆的大小或空闲链表的长度。

4. **使用边界标记的合并**：边界标记是一种内存管理技术，它在每个内存块的开始和结束处存储信息，如块的大小和分配状态。
当一个块被释放时，分配器可以检查相邻的块，如果它们也是空闲的，则可以迅速将它们合并成一个更大的块。
这种合并操作同样是常数时间内完成的，因为它仅涉及对几个边界标记的更新，而不需要遍历整个堆或链表。

当我们说一个操作可以在常数时间内完成，意味着这个操作的执行时间不依赖于数据的大小或复杂度，而是一个固定的时间量。在内存分配器的上下文中，让我们具体看看为什么在使用LIFO顺序和边界标记的情况下，释放一个块和合并空闲块可以在常数时间内完成：

1. **释放操作**：
   - 当一个内存块被释放时，在LIFO（后进先出）策略下，这个块被放置在空闲链表的开始处。
   - 这个操作涉及到更新链表的头部指针，使其指向新释放的块，并更新该块内的指针以指向之前的头部块（在双向链表中还要更新反向指针）。
   - 这些更新是固定数量的内存写操作，与空闲链表的长度或堆的总大小无关。
   - 因此，无论堆的大小或空闲链表的长度如何，这些操作的时间开销都是固定的，即常数时间。

2. **合并操作**：
   - 当使用边界标记时，每个内存块在其开始和结束处都有一个标记，指示块的大小和分配状态。
   - 在释放一个块时，分配器会检查相邻的块。如果这些相邻块也是空闲的，它们可以被合并成一个更大的空闲块。
   - 这个合并操作涉及检查固定的边界位置（即当前块的前后边界）并更新边界标记。
   - 由于只需要检查和更新几个固定位置的标记，这个过程的时间开销与堆的大小或空闲链表的长度无关，是常数时间操作。

因此，无论堆中有多少块或空闲链表有多长，释放操作和合并操作的时间开销都是固定的。
这就是为什么在使用LIFO顺序和边界标记的内存分配器中，这些操作可以在常数时间内完成的原因。
====================================================================================================

另一种方法是按照地址顺序来维护链表，其中链表中每个块的地址都小于它后继的地址。
在这种情况下，释放一个块需要线性时间的搜索来定位合适的前驱。平衡点在于，按照地址排序的首次适配比 LIFO 排序的首次适配有更高的内存利用率，接近最佳适配的利用率。

====================================================================================================
您描述的是一种内存分配策略，其中空闲块链表是按照内存地址的顺序排列的。
这种方法与LIFO（后进先出）策略有显著的不同，并且带来了不同的性能和内存利用率特点。让我们详细探讨这种方法的工作原理和其优缺点：

### 地址顺序链表的工作原理

1. **链表顺序**：在这种方法中，空闲内存块在链表中按照它们在内存中的地址顺序排列。这意味着链表中每个块的地址都小于它后继（即下一个）块的地址。

2. **释放操作**：
   - 当释放一个内存块时，需要将该块插入到地址排序的空闲链表中正确的位置。
   - 为了找到正确的插入位置，分配器需要遍历链表，直到找到一个地址比要释放的块大的块。这需要与链表中的块进行比较，以确定正确的插入点。
   - 因此，这个过程的时间复杂度是线性的，即与链表的长度成正比。

### 地址顺序链表的优点

1. **内存利用率**：按地址排序的首次适配策略可以提高内存利用率。这是因为它减少了内存碎片，特别是减少了所谓的“外部碎片”（即分配器无法利用的小空闲块）。

2. **接近最佳适配**：与LIFO排序的首次适配相比，按地址排序的首次适配在内存利用率上更接近于最佳适配策略。最佳适配策略选择最小的足够大的空闲块进行分配，以尽量减少剩余的空闲块大小，从而提高利用率。

### 地址顺序链表的缺点

1. **性能开销**：在释放块时，需要在链表中进行线性搜索以找到合适的位置，这可能导致性能开销，特别是在链表很长的情况下。

2. **可能的碎片化**：虽然按地址排序的链表在减少外部碎片方面表现更好，但它可能导致一定程度的内部碎片（即空闲块内部未被利用的空间），尤其是当分配的块大小不一时。

### 结论

综上所述，按地址顺序维护空闲块链表的方法在内存利用率上优于LIFO策略，但这是以牺牲释放操作的性能为代价的。这种方法更适合那些内存碎片化对性能影响较大的应用，或者在内存资源较紧张的环境中使用。选择哪种策略取决于特定应用的需求和限制，如性能、内存利用率以及分配和释放操作的频率。
====================================================================================================

一般而言，显式链表的缺点是空闲块必须足够大，以包含所有需要的指针，以及头部和可能的脚部。这就导致了更大的最小块大小，也潜在地提高了内部碎片的程度。

# 分离的空闲链表
就像我们已经看到的，一个使用`单向空闲块链表`的分配器需要与空闲块数量呈线性关系的时间来分配块。
一种流行的减少分配时间的方法，通常称为分离存储（`segregated storage`），就是维护多个空闲链表，其中每个链表中的块有大致相等的大小。
一般的思路是将所有可能的块大小分成一些等价类，也叫做大小类（`size class`）。有很多种方式来定义大小类。例如，我们可以根据 2 的幕来划分块大小：
`{1} {2} {3,4} {5~8} ... {1025~2048} {4097~正无穷}`

或者我们可以将小的块分派到它们自己的大小类里，而将大块按照 2 的幂分类：
`{1} {2} {3} {4} {5} ... {1025~2048} {4097~正无穷}`

分配器维护着一个空闲链表数组，每个大小类一个空闲链表，按照大小的升序排列。当分配器需要一个大小为 n 的块时，它就搜索相应的空闲链表。
如果不能找到合适的块与之匹配，它就搜索下一个链表，以此类推。
====================================================================================================
分离的空闲链表是一种内存管理策略，它旨在提高内存分配的效率。这种方法可以用一个日常生活中的比喻来通俗解释：

想象你经营着一个仓库，里面存放着各种大小的盒子。当客户来到仓库，他们通常会要求特定大小的盒子。
如果你把所有大小的盒子都放在一个巨大的堆里，每次有人需要一个特定大小的盒子时，你都需要挨个检查整个堆，这非常耗时。

为了解决这个问题，你决定把仓库里的盒子按大小分类，每种大小的盒子都放在不同的架子上。例如，所有小盒子放在一个架子上，中等大小的盒子放在另一个架子上，以此类推。这样，当客户要求一个特定大小的盒子时，你只需要去检查存放相应大小盒子的架子，而不是整个仓库。

这就是分离的空闲链表在内存管理中的原理。内存分配器维护着多个空闲链表，每个链表只存放大致相同大小的内存块。这样，当程序请求特定大小的内存块时，分配器只需检查对应大小类的链表。如果在该大小类中找不到合适的块，它就会查看下一个更大的大小类。这大大减少了查找合适内存块所需的时间，因为分配器不必遍历整个内存，只需在少数几个相关的链表中搜索。

这种方法特别适用于那些频繁请求不同大小内存块的场景，因为它能更快地找到合适大小的空闲块，同时减少内存碎片。通过这种方式，内存分配器在效率和速度上都得到了提升。
====================================================================================================

有关动态内存分配的文献描述了几十种分离存储方法，主要的区别在于它们如何定义大小类，何时进行合并，何时向操作系统请求额外的堆内存，是否允许分割，等等。
为了使你大致了解有哪些可能性，我们会描述两种基本的方法：简单分离存储（simple segregated storage）和分离适配（segregated fit）。

## 1. 简单分离适配 `simple segregated storage`
使用简单分离存储，每个大小类的空闲链表包含大小相等的块，每个块的大小就是这个大小类中最大元素的大小。
例如，如果某个大小类定义为 {17 ~ 32}，那么这个类的空闲链表全由大小为 32 的块组成。

为了分配一个给定大小的块，我们检査相应的空闲链表。如果链表非空，我们简单地分配其中第一块的全部。空闲块是不会分割以满足分配请求的。
...
回答：
在基于简单分离存储的分配器中，一种可能导致严重外部碎片的引用模式是“交替大小请求”。这个模式涉及到反复请求两种不同大小的内存块，然后释放其中一种大小的块。例如：
1. **小块和大块交替分配**：首先分配一小块内存，然后分配一大块内存，接着再分配一小块，如此循环。
2. **释放特定大小的块**：在多次分配后，释放所有小块或大块内存，但保留另一种大小的块。
这种模式的结果是，内存中将留下许多不连续的、大小不一的空闲块。小块内存的释放会在大块内存之间留下许多小空间，这些小空间可能无法合并或不足以满足更大的内存请求，从而导致外部碎片。

外部碎片是指分配器无法利用的、分散在内存中的空闲空间。在这种情况下，尽管总的空闲内存量可能相对较大，但由于其分散性和小块特性，无法有效地用于较大的内存请求，导致资源浪费和效率降低。
...
====================================================================================================
你提供的是分离适配（Segregated Fit）内存分配器的一个详细描述，其中包括分配和释放内存块的过程。我将这个过程用更通俗的语言解释如下：

### 分配内存块

1. **确定大小类**：首先，根据需要分配的内存块的大小，确定它属于哪个大小类。大小类就像是不同的“盒子大小”，每个盒子代表一个内存大小范围。

2. **搜索合适的块**：接下来，在与这个大小类关联的空闲链表中查找合适的内存块。这个链表就像是一个存放特定大小盒子的货架。我们从这个货架上找到一个合适的盒子（即内存块）。

3. **分割内存块**（可选）：如果找到的内存块比需要的大，就将它分割成两部分，一部分用于满足当前的内存需求，另一部分则返回到合适的大小类中，以备将来使用。

4. **寻找更大的块**：如果在当前大小类中找不到合适的块，就去下一个更大的大小类中寻找，直到找到合适的块。

5. **请求操作系统内存**：如果所有大小类中都找不到合适的块，那么向操作系统请求额外的堆内存。从这个新的堆内存中分配一个块，并将剩余部分放回合适的大小类中。

### 释放内存块

1. **合并内存块**：当一个内存块不再需要时，它将被释放回内存池。如果与这个块相邻的块也是空闲的，那么这些块将被合并成一个更大的块，以提高空间利用率。

2. **放回相应的链表**：合并后的块被放回到与其大小相匹配的空闲链表中，等待被重新分配。

这种方法的关键在于，通过维护不同大小类的内存块，分离适配分配器能够快速定位到合适大小的内存块，减少了搜索时间，提高了内存利用率。同时，通过合并空闲块，减少内存碎片，进一步提高效率。
====================================================================================================

## 3. 伙伴系统 `Buddy System`

分离适配中的伙伴系统（Buddy System）是一种特殊的内存分配和回收策略。
这个系统的核心思想是将可用内存分成一系列大小是2的幂次方的块。每个块都有一个“伙伴”——与它大小相同并且地址相邻的另一个块。这种系统设计的主要目的是简化内存分配和合并空闲块的过程。

### 工作原理

1. **内存块的分割**：在伙伴系统中，当一个程序请求内存时，系统会提供一个大小至少与请求相等的最小块。如果没有恰好大小的块可用，一个更大的块会被分割成两个大小相等的小块，这两个小块互为伙伴。

2. **内存分配**：系统会分配其中一个块给请求者，而另一个块则保持空闲，可以被后续的内存请求使用。

3. **内存释放和合并**：当一个块被释放时，系统会检查它的伙伴块是否也是空闲的。如果是，这两个伙伴块就会被合并成一个更大的块。这个过程会递归进行，直到不能再合并为止。

### 特点

- **高效的合并操作**：由于每个块都有一个明确的伙伴，合并空闲块变得非常高效。这减少了内存碎片，提高了内存利用率。

- **快速分配**：找到合适大小的块通常很快，因为块的大小是固定且有序的。

- **内存碎片问题**：虽然伙伴系统减少了内存碎片，但在某些情况下仍可能出现内存浪费。例如，如果请求的大小稍微超过某个2的幂次方，分配器可能不得不分配下一个更大的2的幂次方大小的块。

- **适合特定的用例**：伙伴系统特别适用于那些内存请求大小相对固定且变化不大的场景。

综上，伙伴系统是一种在分离适配内存管理中有效减少碎片和提高分配效率的方法，尤其适合于特定类型的应用和内存使用模式。

伙伴系统（buddy system）是分离适配的一种特例，其中每个大小类都是 2 的籍。基本的思路是假设一个堆的大小 `2^m` 为个字，我们为每个块大小 `2^k` 维护一个分离空闲链表，其中 `0 <= k <= m`。请求块大小向上舍入到最接近的 2 的幂。最开始时，只有一个大小为 `2^m` 个字的空闲块。

为了分配一个大小为的块，我们找到第一个可用的、大小为`2^j`的块，其中 `k <= j <= m`。如果 `j = k`，那么我们就完成了。
否则，我们递归地二分割这个块，直到当我们进行这样的分割时，每个剩下的半块（也叫做伙伴）被放置在相应的空闲链表中。
